...
impl<T: CopyTerm + TTerm + Clone, R: BufRead> DynSynTripleSource<T, R> {
        ...
        F: FnMut(StreamedTriple<ByValue<SliceTriple<T>>>) -> Result<(), SinkErr>,
    {
        adapt_stream_result(qs.try_for_some_quad(&mut |q| {
            let in_graph = match (q.g(), virtual_default_graph_iri) {
        ...
        E: Error,
    {
        match &mut self.inner_source {
            InnerStatementSource::FNQuads(qs) => {
                Self::map_from_rio_quad_source(qs, f, &self.quad_source_virtual_default_graph_iri)
            }
            ...
            InnerStatementSource::FNTriples(ts) => Self::map_from_rio_triple_source(ts, f),

            InnerStatementSource::FTurtle(ts) => Self::map_from_rio_triple_source(ts, f),

            InnerStatementSource::FRdfXml(ts) => Self::map_from_rio_triple_source(ts, f),
        }
    }
}
